...
| `about-screed-finish.jpg` | §2 About CIM, right column | https://www.pexels.com/photo/37121405/ (Pexels, free license) | CIM students in the concrete lab or classroom — cylinder testing, slump test, mix design work. This frame stretches to the text column height at desktop, so a wide photo is cropped horizontally (never vertically): the window is set by `.split__frame--about img { object-position: 40% center }`. Re-check that anchor when the photo changes. **Note:** in this stand-in the lead worker's head is above the top edge of the original frame — the source is cropped that way, not the layout. The replacement should show whole people. |
