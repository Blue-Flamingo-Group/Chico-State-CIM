...
| `about-slab-inspection.jpg` | §2 About CIM, right column | https://www.pexels.com/photo/17395035/ (Pexels, free license) | CIM students in the concrete lab or classroom — cylinder testing, slump test, mix design work. This frame stretches to the text column height at desktop, so a wide photo is cropped horizontally and never vertically: the window is set by `.split__frame--about img { object-position: 16% center }`. The subject sits left of centre, and the desktop window is only ~45% of the source width, so re-check that anchor whenever the photo changes. |
